...
${toolResults ? `## 工具数据\n${toolResults}\n\n请基于以上数据进行分析，引用具体数字。` : '（本次未调用工具，请基于用户复盘内容直接分析）'}
...
如果有遗漏，补充具体内容（不超过 200 字）。`
  ...
  // 如果反思发现需要补充，拼接最终回复
  let finalReply = analysisReply
  if (reflectionReply && !reflectionReply.includes('无需补充') && !reflectionReply.includes('质量达标')) {
  ...
  return {
    ...
    metadata: {
      ...
      reflectionApplied: !reflectionReply.includes('无需补充'),
      phases: ['tools', 'analysis', 'reflection']
    }
  }
}
